  ...
  async execute(sock, msg, args, extra) {
    ...
    if (action === 'status') {
      return reply(
        ...
        `✏️ Edit Mode: ${getGroupAutoReplyEditMode() ? '🟢 ON (edits previous reply)' : '🔴 OFF (sends fresh reply each time)'}\n\n` +
        `📨 Current message:\n_${getGroupAutoReplyMessage()}_\n\n` +
    ...
    if (action === 'edit') {
      ...
      if (sub === 'on') {
        setGroupAutoReplyEditMode(true);
        return reply(
          `✏️ *Edit Mode — ON*\n\n` +
          `Bot will now *edit* its previous auto-reply instead of sending a new one each time.\n\n` +
          `_Use .groupautoreply edit off to send a fresh reply every time._`
        );
      }
      if (sub === 'off') {
        setGroupAutoReplyEditMode(false);
        return reply(
          `✏️ *Edit Mode — OFF*\n\n` +
          `Bot will now send a *fresh reply* every time it is triggered.\n\n` +
          `_Use .groupautoreply edit on to edit the previous reply instead._`
        );
      }
      // Show current state if no sub-arg
      return reply(
        `✏️ *Edit Mode — Current: ${getGroupAutoReplyEditMode() ? '🟢 ON' : '🔴 OFF'}*\n\n` +
        `• *.groupautoreply edit on* — Edit previous reply each time\n` +
        `• *.groupautoreply edit off* — Send fresh reply every time (recommended)`
      );
    }
    ...
    return reply(
      ...
      `• *.groupautoreply edit on/off* — Toggle edit vs fresh reply mode\n` +
