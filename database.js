...
// ── Per-User DM Auto Reply Override ──────────────────────────────────────────
// Stored in users.json under each userId as:
//   dmAutoReplyOverride: true  → forced ON for this user
//   dmAutoReplyOverride: false → forced OFF for this user
//   (absent / undefined)       → follow global dmAutoReply setting

const getDmAutoReplyOverride = (userId) => {
  const users = readDB(USERS_DB);
  const clean = String(userId).split('@')[0];
  const key = `${clean}@s.whatsapp.net`;
  const entry = users[key] || users[clean];
  if (!entry) return null;
  if (entry.dmAutoReplyOverride === true) return true;
  if (entry.dmAutoReplyOverride === false) return false;
  return null;
};

const setDmAutoReplyOverride = (userId, value) => {
  // value: true | false | null (null = remove override, follow global)
  const users = readDB(USERS_DB);
  const clean = String(userId).split('@')[0];
  const key = `${clean}@s.whatsapp.net`;
  if (!users[key]) {
    users[key] = { registered: Date.now(), premium: false, banned: false };
  }
  if (value === null) {
    delete users[key].dmAutoReplyOverride;
  } else {
    users[key].dmAutoReplyOverride = value;
  }
  return writeDB(USERS_DB, users);
};

// Returns true/false for whether DM auto reply is active for a specific sender.
const isDmAutoReplyActive = (userId) => {
  const override = getDmAutoReplyOverride(userId);
  if (override === true) return true;
  if (override === false) return false;
  return getDmAutoReply(); // fall back to global
};
...
// ── Group Auto Reply Edit Mode ────────────────────────────────────────────────
// When ON  → bot edits its previous reply instead of sending a new message.
// When OFF → bot always sends a fresh reply every time (default).

const getGroupAutoReplyEditMode = () => {
  delete require.cache[require.resolve('./config')];
  return require('./config').groupAutoReplyEditMode === true;
};

const setGroupAutoReplyEditMode = (value) => {
  const fs = require('fs');
  const path = require('path');
  const cfgPath = path.join(__dirname, 'config.js');
  let src = fs.readFileSync(cfgPath, 'utf8');
  src = src.replace(/(groupAutoReplyEditMode\s*:\s*)(true|false)/, `$1${value}`);
  fs.writeFileSync(cfgPath, src, 'utf8');
  delete require.cache[require.resolve('./config')];
};
...
module.exports = {
  ...
  // Group Auto Reply Edit Mode
  getGroupAutoReplyEditMode,
  setGroupAutoReplyEditMode,
  // Ban system
  getBannedNumbers,
  banNumber,
  unbanNumber,
