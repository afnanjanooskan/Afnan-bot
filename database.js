...
// ── DM Off List helpers (per-chat exceptions) ────────────────────────────────
// Stores JIDs that should NOT receive auto-reply even when global is ON.

const getDmOffList = () => {
  delete require.cache[require.resolve('./config')];
  return require('./config').dmOffList || [];
};

const addToDmOffList = (jid) => {
  const fs   = require('fs');
  const path = require('path');
  const cfgPath = path.join(__dirname, 'config.js');
  let list = getDmOffList();
  if (!list.includes(jid)) {
    list.push(jid);
    let src = fs.readFileSync(cfgPath, 'utf8');
    src = src.replace(/(dmOffList\s*:\s*)\[([^\]]*)\]/, `$1${JSON.stringify(list)}`);
    fs.writeFileSync(cfgPath, src, 'utf8');
    delete require.cache[require.resolve('./config')];
  }
};

const removeFromDmOffList = (jid) => {
  const fs   = require('fs');
  const path = require('path');
  const cfgPath = path.join(__dirname, 'config.js');
  let list = getDmOffList().filter(id => id !== jid);
  let src = fs.readFileSync(cfgPath, 'utf8');
  src = src.replace(/(dmOffList\s*:\s*)\[([^\]]*)\]/, `$1${JSON.stringify(list)}`);
  fs.writeFileSync(cfgPath, src, 'utf8');
  delete require.cache[require.resolve('./config')];
...
const isInDmOffList = (jid) => {
  return getDmOffList().includes(jid);
};
