...
async function startBot() {
  ...
  sock.ev.on('messages.upsert', ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // ── Group Auto Reply ─────────────────────────────────────────────────
      // Runs inside messages.upsert. Uses IIFE to capture msg+from safely
      // (avoids closure bug in for..of loop). Independent from DM Auto Reply.
      ;(async (msg, from) => {
        try {
          // Only group messages, never bot's own messages
          ...
          // Check if group auto reply is active for this group
          if (!database.isGroupAutoReplyActive(from)) return;

          // ── Extract mentioned JIDs (handles all Baileys message shapes) ──
          const m = msg.message || {};
          const getMentions = (obj) => {
            if (!obj || typeof obj !== 'object') return [];
            // Check top-level contextInfo
            if (Array.isArray(obj.contextInfo?.mentionedJid) && obj.contextInfo.mentionedJid.length)
              return obj.contextInfo.mentionedJid;
            // Check one level deeper (extendedTextMessage, imageMessage, etc.)
            for (const val of Object.values(obj)) {
              if (val && typeof val === 'object' && !Array.isArray(val)) {
                if (Array.isArray(val.contextInfo?.mentionedJid) && val.contextInfo.mentionedJid.length)
                  return val.contextInfo.mentionedJid;
              }
            }
            return [];
          };
          const mentionedJids = getMentions(m);

          // ── Message body ─────────────────────────────────────────────────
          const inner = m.extendedTextMessage || m.imageMessage || m.videoMessage
            || m.documentMessage || m.audioMessage || {};
          const body = (
            m.conversation || inner.text || inner.caption || ''
          ).toLowerCase().trim();

          // ── Bot number ───────────────────────────────────────────────────
          const botNumRaw = (sock.user?.id || '').split(':')[0];

          // ── Trigger A: Bot directly mentioned ────────────────────────────
          const botMentioned = mentionedJids.some(jid =>
            jid.split('@')[0].split(':')[0] === botNumRaw
          );

          // ── Trigger B: Any owner number mentioned ─────────────────────────
          const ownerMentioned = mentionedJids.some(jid =>
            ownerNumbers.includes(jid.split('@')[0].split(':')[0])
          );

          // ── Trigger C/D: @all or @everyone literally typed ────────────────
          const allTagUsed = body.includes('@all') || body.includes('@everyone');

          // ── Trigger E: Admin mass mention (tagall style) ──────────────────
          let adminMassMention = false;
          if (!botMentioned && !ownerMentioned && !allTagUsed && mentionedJids.length >= 5) {
            try {
              ...
              if (total > 0 && mentionedJids.length >= Math.floor(total / 2)) {
                ...
                if (senderParticipant?.admin) adminMassMention = true;
              }
            } catch (_) {}
          }

          const shouldReply = botMentioned || ownerMentioned || allTagUsed || adminMassMention;
          ...
          if (prevKey) {
            // Option A: Try to edit previous auto-reply message
            try {
              ...
              // Edit not supported — fall through to Option B
            }
            // Option B: Delete old then send fresh
            try { await sock.sendMessage(from, { delete: prevKey }); } catch (_) {}
          }

          // Send fresh reply quoted to the triggering message
          const sent = await sock.sendMessage(from, {
            text: replyText
          }, { quoted: msg });

          if (sent?.key) garLastReply.set(from, sent.key);

        } catch (e) {
          // Silently ignore group auto reply errors
        }
      })(msg, from);
