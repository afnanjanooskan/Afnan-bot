...
const handleMessage = async (sock, msg) => {
  try {
    ...
    // ── DM Auto Reply — fires BEFORE content check so it works on all message types ──
    ...
    // Chats in dmOffList are silenced even when global is ON.
    if (!isGroup && !msg.key.fromMe) {
      const { getDmAutoReply, getDmAutoReplyMessage, isInDmOffList } = require('./database');
      if (getDmAutoReply()) {
        // Only reply to real users, not to broadcast/status
        if (from !== 'status@broadcast' && !from.endsWith('@g.us')) {
          // Check per-chat exception list
          if (!isInDmOffList(from)) {
