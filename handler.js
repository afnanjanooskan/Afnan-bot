...
const handleMessage = async (sock, msg) => {
  try {
    ...
    // ── Group Auto Reply ─────────────────────────────────────────────────────
    // Fires when bot is mentioned/tagged OR when @all is used by anyone in group.
    // Completely independent from DM Auto Reply. Never affects private chat.
    // Priority: group-level override → global setting.
    if (isGroup && !msg.key.fromMe) {
      try {
        const { isGroupAutoReplyActive, getGroupAutoReplyMessage } = require('./database');
        if (isGroupAutoReplyActive(from)) {
          const botNumber = sock.user.id.split(':')[0] + '@s.whatsapp.net';
          const botNumberRaw = sock.user.id.split(':')[0];

          // Detect all mentioned JIDs in this message
          const mentionedJids = content?.extendedTextMessage?.contextInfo?.mentionedJid
            || content?.contextInfo?.mentionedJid
            || msg.message?.extendedTextMessage?.contextInfo?.mentionedJid
            || [];

          // Check if bot was mentioned/tagged directly
          const botMentioned = mentionedJids.some(jid => {
            const num = jid.split('@')[0].split(':')[0];
            return num === botNumberRaw;
          });

          // Check if @all / @everyone was used (body contains @all or all group members mentioned)
          const bodyLower = body.toLowerCase();
          const allTagUsed = bodyLower.includes('@all') || bodyLower.includes('@everyone');

          // Also check if all group participants were bulk-mentioned (tagall command style)
          const allMembersTagged = groupMetadata &&
            mentionedJids.length > 0 &&
            mentionedJids.length >= (groupMetadata.participants?.length || 0) - 1;

          if (botMentioned || allTagUsed || allMembersTagged) {
            await sock.sendMessage(from, {
              text: getGroupAutoReplyMessage()
            }, { quoted: msg });
            // Do NOT return — allow other processing (anti-spam, commands etc.) to continue
          }
        }
      } catch (e) {
